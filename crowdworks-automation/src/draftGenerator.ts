...
function buildSystemPrompt(short: boolean, applicationRules: string | undefined): string {
  const base =
    ...
    "原文中の実績紹介段落(プロフィールの実績・制作物名を紹介している段落)と、それに先立つポートフォリオURLの掲載(公式サイト・ポートフォリオサイトの見出しとURL)は、原文どおりの位置・文言のまま原則として必ず維持すること。特に業務効率化・社内システム・業務ツール・Webアプリ等の案件では省略しないこと。あなたの判断だけでこれらを削除・省略しないこと。" +
    ...
    "ポートフォリオURLを本文に掲載する場合は、プロフィールのportfolioに登録されている項目だけを、登録されている名前どおりに使うこと。URLや案件内容から実績名を推測したり、公式サイト・ポートフォリオサイトのURLを、サイト内で紹介されている個別の制作物名であるかのように誤表記したりしないこと。原文中の個々の制作物名への言及に対して、その制作物専用の個別URLを勝手に追加しないこと(プロフィールのportfolioに、その制作物専用のURLとして登録されていない限り、URLを書き足さないこと)。クライアントから個別の制作物のURLを明示的に求められた場合のみ、URLを創作せず「本人確認が必要」と書くこと。" +
    ...
    "「〜に携わっており」「〜業務を担当しており」のような表現は有償の顧客実務経験と誤解されるため、プロフィールに明記された内容が個人開発・自己制作である場合は「〜に取り組んでおり」のように、実務委託ではなく自主的な取り組みだと分かる表現にすること。" +
    "ポートフォリオはプロフィールに実在するURLとしてリストされたものだけを使用し、新しいURLを創作しないこと。案件に関連する実績を最大2件までに絞ること。" +
    "プロフィールの稼働時間が特定の単位(週単位・月単位等)でしか明記されていない場合、別の単位へ勝手に掛け算・換算しないこと。稼働時間の話をする際は、プロフィールに明記された単位をそのまま使い、案件側の単位と異なる場合は換算せず「(プロフィールの単位)程度の稼働が可能です。案件で想定されている(案件側の単位)程度であれば、対応可能な範囲と考えております」のように、単位を変えずに両方を併記する形にすること。" +
    ...
    "questionsToConfirmおよび本文中の確認事項は、「応募してよいか」「未経験でも応募可能か」のような可否を尋ねる質問にせず、対象業務フローの範囲、既存システム環境の有無、開発範囲・納品形式など、実作業を進める上で必要な内容を中心にすること。" +
